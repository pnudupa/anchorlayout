...
public:
    ...
    enum OffsetDirection { OD_Auto=-2, OD_Left=-1, OD_Right=1, OD_Up=OD_Left, OD_Down=OD_Right };
    AnchorLine *customLine(Qt::Orientation orientation, qreal percent, OffsetDirection offsetDirection=OD_Auto);
...
public:
    enum Edge { LeftEdge, TopEdge, RightEdge, BottomEdge, HCenter, VCenter, Horizontal, Vertical };
    ~AnchorLine();
    ...
    bool isVerticalLine() const { return m_edge == LeftEdge || m_edge == RightEdge || m_edge == HCenter || m_edge == Vertical; }
    bool isHorizontalLine() const { return m_edge == TopEdge || m_edge == BottomEdge || m_edge == VCenter || m_edge == Horizontal; }
...
private:
    ...
    enum Relationship { NoRelationship, SiblingRelationship, ParentChildRelationship };
    static Relationship relationship(const AnchorLine *line1, const AnchorLine *line2);
